...
def _build_sitrep_markdown(sitrep_data):
    ...
    lines.append("---")
    lines.append("*Protective Intelligence Assistant SITREP.*")
    return "\n".join(lines)
...
def generate_sitrep_for_poi_escalation(conn, poi_id, assessment):
    ...
    window_start = assessment.get("window_start", "N/A")
    window_end = assessment.get("window_end", "N/A")
    situation_summary = (
        ...
        f"Assessment window: {window_start} to {window_end}."
    )
...
def generate_sitrep_for_facility_breach(conn, alert_id, location_id):
    ...
    sitrep_data = {
        ...
        "situation_summary": situation_summary,
        "threat_assessment": (
            "A threat-related alert is proximate to a protected facility. "

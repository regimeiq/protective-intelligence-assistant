"""Prototype Telegram collector (fixture-first, policy-gated)."""
...
FIXTURE_PATH = Path(__file__).resolve().parents[1] / "fixtures" / "telegram_fixtures.json"


def _enabled():
    return os.getenv("PI_ENABLE_TELEGRAM_COLLECTOR", "0").strip().lower() in {"1", "true", "yes"}
...
def _ensure_source(conn):
    row = conn.execute(
        "SELECT id FROM sources WHERE source_type = 'telegram' AND name = ?",
        ("Telegram Public Channels (Prototype)",),
    ).fetchone()
    if row:
        return int(row["id"])
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, 1)""",
        ("Telegram Public Channels (Prototype)", "https://t.me", "telegram", 0.35),
    )
...
def _resolve_keyword_id(conn, keyword_cache, post, combined_text):
    ...
    weight = float(post.get("keyword_weight", 3.5) or 3.5)
    ...
    if not matched_term:
        matched_term = "telegram threat signal"
    conn.execute(
...
def run_telegram_collector(frequency_snapshot=None):
    conn = get_connection()
    source_id = None
    try:
        source_id = _ensure_source(conn)
        if not _enabled():
            mark_source_skipped(conn, source_id, "PI_ENABLE_TELEGRAM_COLLECTOR not set")
            conn.commit()
            print("Telegram collector skipped (PI_ENABLE_TELEGRAM_COLLECTOR not set).")
            return 0

        posts = _load_fixtures()
        if not posts:
            mark_source_failure(conn, source_id, "telegram fixtures missing or empty")
            conn.commit()
            print("Telegram collector skipped (no fixtures found).")
            return 0
        ...
        for post in posts:
            ...
            # URL-level dedup avoids multiple keyword variants per post.
        ...
        conn.commit()
        print(f"Telegram collector complete. {created} new alerts, {duplicates} duplicates skipped.")
        return created
    except Exception as exc:
        if source_id is not None:
            mark_source_failure(conn, source_id, f"telegram collector error: {exc!r}")
            conn.commit()
        print(f"Telegram collector failed: {exc}")
        return 0
    finally:
        conn.close()
